...
- **Animate mode:** up to 12 onion-skin frames, each with up to 3 layers (visibility, reorder, background fill — same model as Draw mode). Choose how new frames start: **Copy** duplicates the last frame so you can nudge or tweak it, or **Trace** starts blank (keeping each layer's background) with the onion-skin ghost as a guide, for classic frame-by-frame drawing. Includes a preview-loop button to check pacing before posting, and per-frame/per-layer delete/undo/clear.
